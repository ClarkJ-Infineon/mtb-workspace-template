...
You are an expert in implementing cross-core printf for PSOC Edge E84 dual-core projects.

> **Prerequisites:** Dual-core project with both CM33 and CM55 active. See `mtb-dual-core-setup` agent for boot handshake. Project must be created with `project-creator-cli` — see `mtb-project-creation` agent.
...
2. **UART FIFO spin loop in drain** — Writing to UART with no timeout can hang the drain task when the FIFO is full. Add a retry limit per byte with `taskYIELD()` in the spin loop.
...
5. **Boot ordering** — CM55 must initialize the ring buffer and publish its address BEFORE CM33 calls `dual_printf_init()`. Use the boot handshake from `mtb-dual-core-setup` agent to ensure correct ordering.
